// Brawl perf gate: fps at deviceScaleFactor 2 (pixelRatio 2) during a live
// free-for-all with skills firing. Mirrors perf-duel-dpr2.mjs.
import { chromium } from 'playwright-core'

const port = process.env.IPL_PORT || '5187'
const browser = await chromium.launch({ channel: 'chrome', headless: true })
const page = await browser.newPage({ viewport: { width: 1440, height: 810 }, deviceScaleFactor: 2 })
const errors = []
page.on('console', m => { if (m.type() === 'error') errors.push(m.text()) })
page.on('pageerror', e => errors.push(String(e)))
await page.goto(`http://localhost:${port}/?scene=brawl&mute=1`, { waitUntil: 'load' })
...
const pr = await page.evaluate(() => window.__ipl.engine.renderer.getPixelRatio())
console.log(`BRAWL FPS(avg): ${r.fps.toFixed(1)}  p95 frame: ${r.p95.toFixed(1)}ms  pixelRatio: ${pr}  frames: ${r.frames}`)
