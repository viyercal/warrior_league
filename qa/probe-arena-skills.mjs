// probe-arena-skills.mjs — exercises ALL 12 skill archetypes via real QWER
// keypresses across 3 loadouts (localStorage-injected profiles). Asserts
// per-archetype effects: blink teleports, titan scales, ghost fades+restores,
// decoy spawns/expires, zones exist, cooldowns tick.
// Usage: node qa/probe-arena-skills.mjs [port]
...
for (let li = 0; li < LOADOUTS.length; li++) {
  ...
  const st = await page.evaluate(() => {
    ...
    let ghostMats = 0
    s.hero.group.traverse(o => { if (o !== window.__scene.hero.ring && o !== window.__scene.bubble && o.material && o.material.transparent && o.material.opacity <= 0.33) ghostMats++ })
    return {
  ...
  if (loadout.includes('ghost')) {
    ...
    await page.waitForTimeout(3600) // wait for phase cloak to expire
    const after = await page.evaluate(() => {
      let bad = 0
      window.__scene.hero.group.traverse(o => { if (o !== window.__scene.hero.ring && o !== window.__scene.bubble && o.material && o.material.transparent && o.material.opacity <= 0.33) bad++ })
      return { bad, ghostT: window.__scene.ghostT }
    })
...
await browser.close()
console.log(fails.length ? `\nSKILLS PROBE: ${fails.length} FAILURES` : '\nSKILLS PROBE: ALL 12 ARCHETYPES PASS')
